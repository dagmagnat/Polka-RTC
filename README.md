...
```text
telemost-auto-recovery-2026-05-15-3
```
...
## Автовосстановление Telemost каждые 5 минут

В карточке каждого Telemost-клиента добавлена кнопка:

```text
🛟 Автовосст. 5м: ВКЛ/ВЫКЛ
```

Если включить, watchdog будет примерно раз в 5 минут выполнять для этого клиента `Stable restart`.

Это сделано для сценария, когда клиент выключил VPN, через несколько минут снова нажал Start, но канал не восстановился. Автовосстановление периодически пересоздаёт серверную сторону Telemost с теми же `ROOM_ID`, `AUTH_KEY` и `CLIENT_ID`.

Важно:

```text
1. По умолчанию автовосстановление выключено.
2. Включается отдельно для каждого клиента.
3. Может кратко прерывать активное соединение, поэтому включайте только там, где это действительно нужно.
4. Если клиент остановлен кнопкой “Остановить”, watchdog его не поднимет.
```

Проверка:

```bash
grep -R -E 'TELEMOST_AUTO_RECOVERY|TELEMOST_AUTO_RECOVERY_INTERVAL_MINUTES' /etc/olcrtc/clients/*.env
tail -n 100 /var/log/polka-rtc-watchdog.log
```
